...
namespace IFCGeometry2RDF
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length != 1)
            {
                Console.WriteLine("Usage: IFCGeometry2RDF <inputFile>");
                return;
            }
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Failed to load model: " + inputFile);
                    return;
                }
                ...
                foreach (var prGeometry in model.Geometries)
                {
                    ...
                    ifcengine.sdaiGetAttrBN(prGeometry.Value.Instance, "GlobalId", ifcengine.sdaiUNICODE, out IntPtr ptrGlobalId);
                    string? globalId = string.Empty;
                    if (ptrGlobalId != IntPtr.Zero)
                    {
                        globalId = Marshal.PtrToStringUni(ptrGlobalId);
                    }

                    Console.WriteLine($"Processing geometry {index++}: {globalId}...");
                    ...
                    if (prGeometry.Value.OwlInstance != 0 && !string.IsNullOrEmpty(globalId))
                    {
                        ...
                        var safeEntityName = ToSafeTurtleLocalName(prGeometry.Value.Entity);
        ...
        private static long CreateMapConversionTransformation(long sdaiModel, double dScale, out double dEastings, out double dNorthings)
        {
